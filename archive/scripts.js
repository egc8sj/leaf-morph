// Part one - bar chart
	...
	d3.json("data/barchart.json", function(error, data) {	// Change this file location to use a different file
		...
		data.forEach(function(d) {
			...
			d.Width = +d.Width; // The + sign here converts data to a number. This is a fail-safe to make sure the data is consistent, since the data needs to be numeric for the scaleLinear() function to work. 
		});

		// Scale the range of the data
		x.domain(data.map(function(d) { return d.Latitude; }));
		y.domain([0, d3.max(data, function(d) { return d.Width; })]);
		...
		.attr("y", function(d) { return y(d.Width); })
		.attr("width", x.bandwidth())
		.attr("height", function(d) { return bcHeight - y(d.Width); });
	});
